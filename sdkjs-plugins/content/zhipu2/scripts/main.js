...
(function (window, undefined) {
    ...
    function generateImageFromText(text) {
        ...
        window.Asc.plugin.executeMethod('StartAction', ['Block', 'Zhipu AI is generating ']);
        zhipuImgRequest(text).then(response => {
            window.Asc.plugin.executeMethod('EndAction', ['Block', 'Zhipu AI is generating ']);
    ...
    function createUserMessageFrame() {
        ...
        imgElement.alt = `user message avatar`;
    ...
    function chatRequest(request) {
        let prompt = request;
        let systemMessage = "You are a good word document assistant, please help user solve problems."
        let config = {model: localStorage.getItem('model')}
        zhipuChatRequest(prompt, systemMessage, true, config)
            .then(reader => {
                console.log("SSE请求成功");
