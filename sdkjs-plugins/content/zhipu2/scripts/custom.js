(function (window, undefined) {
    ...
    const models = [
        ...
        {id: "glm-4V", text: "glm-4V", modelDescription: "Deep integration of visual language features is realized without sacrificing any NLP task performance. Support visual question answering, image captioning, visual positioning, complex object detection and other image understanding tasks."},
        {id: "glm-4-alltools", text: "glm-4-alltools", modelDescription: "AllTools model by planning user questions, choosing the right tools, step by step analysis and take the next action, and finally complete a complex task."}
    ]
    ...
    window.Asc.plugin.init = function() {
        ...
        elements.btnSubmit.onclick = function() {
            ...
            let systemMessage = 'Please provide a response to the following prompt:';
            let stream = true;
            ...
            zhipuChatRequest(prompt, systemMessage, stream, config).then(reader => {
    ...
    function fetchModels() {
        ...
        if ($('#sel_models').find('option[value=glm-4v]').length) {
            $('#sel_models').val('glm-4v').trigger('change');
        }
